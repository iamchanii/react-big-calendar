  ...
  function DayColumn() {
    ...
    _this.renderEvents = function () {
      ...
      for (var i = 0; i < styledEvents.length; ++i) {
        var se1 = styledEvents[i];
        var y1 = se1.style.top;
        var y2 = se1.style.top + se1.style.height;

        for (var j = i + 1; j < styledEvents.length; ++j) {
          var se2 = styledEvents[j];
          var y3 = se2.style.top;
          var y4 = se2.style.top + se2.style.height; // 두 이벤트가 겹칠때
      ...
      for (var _i = 0; _i < styledEvents.length; ++_i) {
        _loop(_i);
      }
      ...
      styledEvents.map(function (se) {
        ...
        function dfs(node, maxLeft, visited) {
          node.friends.forEach(function (f) {
        ...
        allFriends.map(function (f) {
          return f.style.width = width;
        });
        return se;
      });
      styledEvents.map(function (se) {
        var originWidth = se.style.width; // stretch to maximum

        var maxLeft = 0;
        se.friends.forEach(function (f) {
          return maxLeft = Math.max(maxLeft, f.style.left);
        });
        if (maxLeft <= se.style.left) se.style.width = 100 - se.style.left * se.style.width; // padding between events
        // for this feature, `width` is not percentage based unit anymore
        // it will be used with calc()

        var padding = se.style.left === 0 ? 0 : 3;
